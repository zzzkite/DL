#!/usr/bin/env python3
"""
验证提取的数据是否正确 - 适配前20帧→第25帧设置，512x512版本
"""
...
def verify_data():
    ...
    # 加载元数据
    metadata_dir = Path("processed_data_512/metadata")
    ...
    for category in categories:
        ...
        for i, sample in category_samples.head(2).iterrows():
            ...
            try:
                ...
                expected_shape = (20, 512, 512, 3)  # 更新期望形状为512x512
                ...
                expected_target_shape = (512, 512, 3)  # 更新期望形状为512x512
                ...
                # 检查图像质量（可选）
                if input_frames.shape[1] == 512 and input_frames.shape[2] == 512:
                    # 计算图像锐度（拉普拉斯方差）
    ...
    # 检查示例图像
    print(f"\n=== 示例图像检查 ===")
    samples_dir = Path("processed_data_512/samples")
    for category in categories:
    ...
    if info_file.exists():
        ...
        if resolution != "512x512":
            print(f"⚠️  分辨率不匹配: 期望512x512，实际{resolution}")
        else:
            print(f"✅ 分辨率验证通过")
    else:
        print("❌ 数据集信息文件不存在")
