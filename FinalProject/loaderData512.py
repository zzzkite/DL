#!/usr/bin/env python3
"""
PyTorch数据加载器 - 支持任务特定训练 (512x512版本)
"""
...
class TaskSpecificDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            # 加载输入帧（前20帧）
            input_frames = np.load(sample['input_frames_path'])  # (20, 512, 512, 3)
            # 加载目标帧（第25帧）
            target_frame = np.load(sample['target_frame_path'])  # (512, 512, 3)
            ...
            input_frames = input_frames.permute(0, 3, 1, 2)  # (20, 3, 512, 512)
            target_frame = target_frame.permute(2, 0, 1)     # (3, 512, 512)
            ...
            return {
                'input_frames': input_frames,  # (20, 3, 512, 512)
                'target_frame': target_frame,  # (3, 512, 512)
    ...
    def _get_empty_sample(self):
        ...
        return {
            'input_frames': torch.zeros((20, 3, 512, 512)),
            'target_frame': torch.zeros((3, 512, 512)),
            'category': 'error',
            'video_id': 'error',
            'template': 'error',
            'label_text': 'error'
...
def create_task_specific_loaders(task_name='all', batch_size=4, data_path="processed_data_512", num_workers=2):
    """
    创建任务特定的训练、验证和测试数据加载器 (512x512版本)
    
    Args:
        task_name: 任务名称 ('move_object', 'drop_object', 'cover_object', 'all')
        batch_size: 批次大小 (建议减小批次大小以适应高分辨率)
        data_path: 处理数据的路径
        num_workers: 数据加载的进程数
    """
    
    # 数据变换 - 可以添加针对512x512的数据增强
...
# 保留原来的函数用于兼容性
def create_data_loaders(batch_size=4, data_path="processed_data_512", num_workers=2):
    """创建所有任务混合的数据加载器"""
    return create_task_specific_loaders('all', batch_size, data_path, num_workers)

def test_task_specific_loader():
    """测试任务特定数据加载器"""
    print("测试任务特定数据加载器 (512x512版本)...")
...
def estimate_memory_usage(batch_size=4):
    """估算内存使用情况"""
    print("\n=== 内存使用估算 ===")
    
    # 输入帧: (batch_size, 20, 3, 512, 512) - float32
    input_memory = batch_size * 20 * 3 * 512 * 512 * 4 / (1024**3)  # GB
    
    # 目标帧: (batch_size, 3, 512, 512) - float32  
    target_memory = batch_size * 3 * 512 * 512 * 4 / (1024**3)  # GB
    
    print(f"批次大小: {batch_size}")
    print(f"输入帧内存: {input_memory:.2f} GB")
    print(f"目标帧内存: {target_memory:.2f} GB")
    print(f"总内存需求: {input_memory + target_memory:.2f} GB")
    print(f"建议GPU内存: > {(input_memory + target_memory) * 3:.2f} GB (包含模型和梯度)")

if __name__ == "__main__":
    test_task_specific_loader()
    estimate_memory_usage(batch_size=4)